    ...
    <div style={{ display: "flex", gap: 24, maxWidth: 1200, margin: "0 auto", position: "relative" }}>
      ...
      <div style={{ width: 300, flexShrink: 0, position: "relative" }}>
        ...
        {!isPro && (
          <div style={{ position: "absolute", inset: "60px 0 0 0", zIndex: 10, background: "rgba(250,248,245,.85)", backdropFilter: "blur(4px)", borderRadius: 14, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32, textAlign: "center" }}>
            ...
            <h3 style={{ fontFamily: "'Fraunces',serif", fontSize: 18, fontWeight: 700, color: "var(--bois,#3D2E1F)", marginBottom: 8 }}>Passez au plan Pro</h3>
            <p style={{ fontSize: 14, color: "var(--bois-mid,#5C4A3A)", lineHeight: 1.5, marginBottom: 20 }}>Personnalisez votre vitrine avec 4 templates, 8 palettes de couleurs et 4 dispositions photos.</p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {TEMPLATES.map((t) => (
              <button key={t.id} onClick={() => setTemplate(t.id)} style={{
                ...
                <div style={{ fontSize: 10, color: "var(--pierre,#9C958D)", marginTop: 2 }}>{t.description}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Couleurs */}
        <div style={C}>
          <h3 style={{ fontSize: 14, fontWeight: 700, color: "var(--bois,#3D2E1F)", marginBottom: 12 }}>Palette de couleurs</h3>
          ...
          <p style={{ fontSize: 11, color: "var(--pierre,#9C958D)", marginTop: 6 }}>{currentPalette.nom} &mdash; Principale + Accent</p>
        </div>
        ...
        <div style={C}>
          ...
          {isProPlus ? (
            <>
              <p style={{ fontSize: 12, color: "var(--pierre,#9C958D)", marginBottom: 12, lineHeight: 1.5 }}>G&eacute;n&eacute;rez une meta description et des mots-cl&eacute;s optimis&eacute;s pour le r&eacute;f&eacute;rencement.</p>
              <button
                onClick={async () => {
                  setSeoLoading(true);
                  try {
              ...
              {seoResult && (
                <div style={{ marginTop: 12, padding: 12, background: "#F0FDF4", borderRadius: 8, border: "1px solid rgba(22,163,74,.15)" }}>
                  ...
                  <p style={{ fontSize: 12, color: "var(--bois-mid,#5C4A3A)", lineHeight: 1.4 }}>{seoResult.metaDescription}</p>
                  <p style={{ fontSize: 11, fontWeight: 600, color: "#15803d", marginTop: 8, marginBottom: 4 }}>Mots-cl&eacute;s :</p>
                  <p style={{ fontSize: 12, color: "var(--bois-mid,#5C4A3A)", lineHeight: 1.4 }}>{seoResult.keywords}</p>
                </div>
              )}
            </>
          ) : (
            <div style={{ padding: 12, background: "#FAF8F5", borderRadius: 8, textAlign: "center" }}>
              <p style={{ fontSize: 12, color: "var(--pierre,#9C958D)" }}>Disponible avec le plan Business</p>
            </div>
          )}
        </div>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
          <span style={{ fontSize: 14, fontWeight: 600, color: "var(--pierre,#9C958D)" }}>Aper&ccedil;u de votre vitrine</span>
          {vitrineHref !== "#" && (
        ...
        <div style={{ background: "#fff", borderRadius: 16, border: "1px solid var(--sable,#E8D5C0)", overflow: "hidden", boxShadow: "0 4px 24px rgba(28,28,30,.06)" }}>
          {/* Browser bar */}
          <div style={{ padding: "8px 16px", background: "#F7F5F2", borderBottom: "1px solid var(--sable,#E8D5C0)", display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <div style={{ padding: 20 }}>
              <div style={{ background: "#1C1C1E", borderRadius: 12, padding: "28px 24px", marginBottom: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  <span style={{ background: currentPalette.accent, color: "var(--bois,#3D2E1F)", padding: "4px 10px", borderRadius: 12, fontSize: 13, fontWeight: 700, fontFamily: "'Fraunces',serif" }}>4.8</span>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginBottom: 12 }}>
                {[{ v: "15", l: "années" }, { v: "47", l: "avis" }, { v: "25 km", l: "rayon" }].map((s) => (
                  <div key={s.l} style={{ background: "#fff", border: "1px solid var(--sable,#E8D5C0)", borderRadius: 8, padding: 12, textAlign: "center" }}>
                    <div style={{ fontFamily: "'Fraunces',serif", fontSize: 20, fontWeight: 700, color: currentPalette.primary }}>{s.v}</div>
                    <div style={{ fontSize: 10, color: "var(--pierre,#9C958D)" }}>{s.l}</div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ padding: 0 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
                ...
                <div style={{ padding: 24, display: "flex", flexDirection: "column", justifyContent: "center" }}>
                  <span style={{ background: currentPalette.accent, color: "var(--bois,#3D2E1F)", padding: "3px 8px", borderRadius: 8, fontSize: 11, fontWeight: 700, alignSelf: "flex-start", marginBottom: 8 }}>4.8 &#9733;</span>
                  <h2 style={{ fontFamily: "'Fraunces',serif", fontSize: 20, fontWeight: 700 }}>{user?.nomAffichage || "Mon entreprise"}</h2>
                  <p style={{ fontSize: 12, color: "var(--pierre,#9C958D)", marginTop: 4 }}>{user?.metierNom || "Artisan"} &agrave; {user?.ville || "Chambéry"}</p>
              ...
              <div style={{ padding: 16 }}>
                <p style={{ fontSize: 10, fontWeight: 600, color: "var(--pierre,#9C958D)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Galerie masonry</p>
          ...
          {template === "moderne" && (
            <div style={{ padding: 20 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gridTemplateRows: "auto auto", gap: 8 }}>
                ...
                <div style={{ background: "#FAF8F5", borderRadius: 10, padding: 14, border: "1px solid var(--sable,#E8D5C0)" }}>
                  <p style={{ fontSize: 10, color: "var(--bois-mid,#5C4A3A)", lineHeight: 1.4 }}>D&eacute;pannage, r&eacute;novation, chauffage...</p>
                </div>
              </div>
            ...
            <div style={{ padding: 0 }}>
              ...
              <div style={{ padding: 20, textAlign: "center" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 4, marginTop: 8 }}>
                  ...
                  <span style={{ fontSize: 11, color: "var(--pierre,#9C958D)" }}>(47)</span>
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }}>
                <div style={{ padding: 16, background: "#FAF8F5" }}>
                  <p style={{ fontSize: 10, fontWeight: 600, color: "var(--pierre,#9C958D)", marginBottom: 6 }}>PR&Eacute;SENTATION</p>
                  <p style={{ fontSize: 11, color: "var(--bois-mid,#5C4A3A)", lineHeight: 1.4 }}>{user?.metierNom || "Artisan"} &agrave; {user?.ville || "Chambéry"}.</p>
                </div>
          ...
          <div style={{ padding: "8px 20px 12px", textAlign: "center", fontSize: 11, color: "#C5C0B9", borderTop: "1px solid #F7F5F2" }}>
            Template : <strong style={{ color: "var(--bois-mid,#5C4A3A)" }}>{TEMPLATES.find((t) => t.id === template)?.nom}</strong> &middot; Photos : <strong style={{ color: "var(--bois-mid,#5C4A3A)" }}>{PHOTO_LAYOUTS.find((l) => l.id === photoLayout)?.nom}</strong>
          </div>
        </div>
      </div>
    </div>
